...
all_tumor_cells_fpkm <- read.csv("Single-cell-data/FPKM/GSE81861_CRC_tumor_all_cells_FPKM.csv")
...
ggsave(filename = "Outputs/single_cell_library_histogram_pre_process_step_1_scdd_tumor.svg",
...
ggsave(filename = "Outputs/single_cell_library_histogram_pre_process_step_2_scdd_tumor.svg",
       device = "svg", units = "cm", dpi = 300, width = 20, height = 20,
       plot = library_size_plot_2)
...
saveRDS(all_tumor_cells_fpkm$denoised_sc_dataframe,
        file = "Outputs/denoised-colon-and-rectal-single-cell-data_scdd_tumor.rds")
```


```{r}
all_nm_cells_fpkm <- read.csv("Single-cell-data/FPKM/GSE81861_CRC_NM_all_cells_FPKM.csv")
...
all_nm_cells_fpkm <- subset(all_nm_cells_fpkm,
                            select=c(RHC3934__Bcell__.7DEA7B:RHC6187__Macrophage__.FFFF55))
...
ggsave(filename = "Outputs/single_cell_library_histogram_pre_process_step_1_scdd_nm.svg",
       device = "svg", units = "cm", dpi = 300, width = 24, height = 24,
       plot = library_size_plot_1)
...
ggsave(filename = "Outputs/single_cell_library_histogram_pre_process_step_2_scdd_nm.svg",
       device = "svg", units = "cm", dpi = 300, width = 20, height = 20,
       plot = library_size_plot_2)
...
saveRDS(all_nm_cells_fpkm$denoised_sc_dataframe,
        file = "Outputs/denoised-colon-and-rectal-single-cell-data_scdd_nm.rds")
...
saveRDS(scdd_res, file = "Outputs/scdd_res.rds")
write.csv(scdd_res, file = "Outputs/scdd_res.csv")
...
for(gs in gene_sizes){
  current_cox <- cox_model_fitter(my.seed = 1, my.alpha = 1, cox.df = cox_df,
                                  ...
                                  my.dataset = "READ",
                                  my.filename = "Outputs/scdd_coefs.csv")
...
write.csv(max(my_cindices), file = "Outputs/read_scdd_top_cindex.csv")
saveRDS(my_cindices, file = "Outputs/read_scdd_top_cindex.rds")
...
#Loading our single cell data for DESeq2
...
keep_rows <- rowSums(all_tumor_cells_fpkm > 5) > 10
table(keep_rows)
all_tumor_cells_fpkm <- all_tumor_cells_fpkm[keep_rows,]
all_tumor_cells_fpkm <- apply(all_tumor_cells_fpkm, c(1,2), as.numeric)
...
library_size_plot_1 <- ggplot() +
  ...
  theme(panel.background = element_blank(),
        plot.title = element_text(face = "bold", hjust = 0.5))+
  xlab("Library Size Distribution")+
  ylab("Count") + ggtitle("Single-cell Library Size for DESeq 2 Tumor")+
  scale_y_continuous(limits=c(0, 35), expand = c(0, 0))

library_size_plot_1


ggsave(filename = "Outputs/single_cell_library_histogram_pre_process_step_1_deseq2_tumor.svg",
       device = "svg", units = "cm", dpi = 300, width = 24, height = 24,
       plot = library_size_plot_1)
...
#Normalizing the library size
all_tumor_cells_fpkm <- library.size.normalize(t(all_tumor_cells_fpkm),
                                               verbose = TRUE)
all_tumor_cells_fpkm <- t(all_tumor_cells_fpkm)
tumor_col_names <- colnames(all_tumor_cells_fpkm)
tumor_row_names <- rownames(all_tumor_cells_fpkm)
...
library_size_plot_2 <- ggplot() +
  ...
  theme(panel.background = element_blank(),
        plot.title = element_text(face = "bold", hjust = 0.5))+
  xlab("Library Size Distribution")+
  ylab("Count") + ggtitle("Single-cell Library Size Normalized for DESeq2 Tumor")+
  scale_y_continuous(limits=c(0, 35), expand = c(0, 0))

library_size_plot_2

ggsave(filename = "Outputs/single_cell_library_histogram_pre_process_step_2_deseq2_tumor.svg",
       device = "svg", units = "cm", dpi = 300, width = 20, height = 20,
       plot = library_size_plot_2)
...
saveRDS(all_tumor_cells_fpkm$denoised_sc_dataframe,
        file = "Outputs/denoised-colon-and-rectal-single-cell-data_deseq2_tumor.rds")
```
...
keep_rows <- rowSums(all_nm_cells_fpkm > 5) > 10
table(keep_rows)
all_nm_cells_fpkm <- all_nm_cells_fpkm[keep_rows,]
all_nm_cells_fpkm <- apply(all_nm_cells_fpkm, c(1,2), as.numeric)
...
library_size_plot_1 <- ggplot() +
  ...
  theme(panel.background = element_blank(),
        plot.title = element_text(face = "bold", hjust = 0.5))+
  xlab("Library Size Distribution")+
  ylab("Count") + ggtitle("Single-cell Library Size for DESeq2 Normal")+
  scale_y_continuous(limits=c(0, 60), expand = c(0, 0))

library_size_plot_1


ggsave(filename = "Outputs/single_cell_library_histogram_pre_process_step_1_deseq2_nm.svg",
       device = "svg", units = "cm", dpi = 300, width = 24, height = 24,
       plot = library_size_plot_1)
...
#Normalizing the library size
all_nm_cells_fpkm <- library.size.normalize(t(all_nm_cells_fpkm),
                                            verbose = TRUE)
all_nm_cells_fpkm <- t(all_nm_cells_fpkm)
tumor_col_names <- colnames(all_nm_cells_fpkm)
tumor_row_names <- rownames(all_nm_cells_fpkm)
...
library_size_plot_2 <- ggplot() +
  ...
  theme(panel.background = element_blank(),
        plot.title = element_text(face = "bold", hjust = 0.5))+
  xlab("Library Size Distribution")+
  ylab("Count") + ggtitle("Single-cell Library Size Normalized for DESeq2")+
  scale_y_continuous(limits=c(0, 35), expand = c(0, 0))

library_size_plot_2

ggsave(filename = "Outputs/single_cell_library_histogram_pre_process_step_2_deseq2_nm.svg",
       device = "svg", units = "cm", dpi = 300, width = 20, height = 20,
       plot = library_size_plot_2)
...
saveRDS(all_nm_cells_fpkm$denoised_sc_dataframe,
        file = "Outputs/denoised-colon-and-rectal-single-cell-data_deseq2_nm.rds")
...
#Merging them together into one large data frame
#tumor_cells_ds2_df <- all_tumor_cells_fpkm
#tumor_cells_ds2_df <- apply(tumor_cells_ds2_df, 2, as.integer)
#rownames(tumor_cells_ds2_df) <- rownames(tumor_cells_ds2$denoised_sc_dataframe)
#nm_cells_ds2_df <- all_nm_cells_fpkm
#nm_cells_ds2_df <- apply(nm_cells_ds2_df, 2, as.integer)
#rownames(nm_cells_ds2_df) <- rownames(nm_cells_ds2$denoised_sc_dataframe)



# deseq2_df <- merge(all_tumor_cells_fpkm, all_nm_cells_fpkm)
...
test_df <- dplyr::bind_cols(all_tumor_cells_fpkm, all_nm_cells_fpkm)

sce <- SingleCellExperiment(assays=list(counts=cbind(all_tumor_cells_fpkm,
                                                         all_nm_cells_fpkm)),
                            colData=data.frame(condition))
...
## Trying to first merge the two data frames and then do the cleaning and filtering
```{r}
#Loading needed functions for tidying the gene names
gene_name_cleaner <- function(data.to.clean=all_tumor_cells_fpkm_denoised_df){
  data.to.clean <-t(data.to.clean)
...
# nm_names <- rep(paste("NM",1:ncol(all_nm_cells_fpkm$denoised_sc_dataframe)))
# tumor_names <- rep(paste("TM",1:ncol(all_tumor_cells_fpkm$denoised_sc_dataframe)))
...
#Normalizing the library size
deseq2_df <- library.size.normalize(t(deseq2_df),
                                            verbose = TRUE)
deseq2_df <- t(deseq2_df)
tumor_col_names <- colnames(deseq2_df)
head(as.data.frame(deseq2_df))
deseq2_df <- sqrt(deseq2_df)
head(as.data.frame(deseq2_df))
...
ggsave(filename = "Outputs/single_cell_library_histogram_pre_process_step_2_deseq2_df.svg",
       device = "svg", units = "cm", dpi = 300, width = 20, height = 20,
       plot = library_size_plot_2)
...
#Denoising the single-cell data and saving the output
deseq2_df <- magic_denoiser(sc.data = deseq2_df,
                                    magic.seed = 123,
                                    magic.solver = 'approximate')

saveRDS(deseq2_df$denoised_sc_dataframe,
        file = "Outputs/denoised-colon-and-rectal-single-cell-data_deseq2_df.rds")
...
```
```{r}
deseq2_rows <- my_finished_tumor_rownames
deseq2_cols <- colnames(deseq2_df$denoised_sc_dataframe)
deseq2_df_finished <- deseq2_df$denoised_sc_dataframe
deseq2_df_finished <- apply(deseq2_df_finished, c(1,2), as.integer)
deseq2_mat <- as.matrix(deseq2_df_finished)
...
rownames(deseq2_se) <- deseq2_rows
...
zinb_data <- zinb_data[names(zinb_data)[1:1932],]
...
saveRDS(deseq2_genes, file = "Outputs/deseq2_top_genes.rds")
```
...
for(gs in gene_sizes){
  current_cox <- cox_model_fitter(my.seed = 1, my.alpha = 1, cox.df = cox_df,
                                  ...
                                  my.dataset = "READ",
                                  my.filename = "Outputs/deseq2_coefs.csv")
...
write.csv(max(my_cindices), file = "Outputs/read_deseq2_top_cindex.csv")
saveRDS(my_cindices, file = "Outputs/read_deseq2_top_cindex.rds")
...
write.csv(finished_edger, file = "Outputs/edgeR_1927_genes.csv")
saveRDS(finished_edger, file = "Outputs/edgeR_1927_genes.rds")
```
...
tumor_cells_ds2 <- read.csv(file = "Single-cell-data/Counts/GSE81861_CRC_tumor_all_cells_COUNT.csv")
nm_cells_ds2 <- read.csv(file = "Single-cell-data/Counts/GSE81861_CRC_NM_all_cells_COUNT.csv")


rownames(tumor_cells_ds2) <- tumor_cells_ds2$X
tumor_cells_ds2 <- gene_name_cleaner(data.to.clean = tumor_cells_ds2)
tumor_cells_ds2 <- t(tumor_cells_ds2)
tumor_cells_ds2 <- subset(tumor_cells_ds2,
                            select=c(RHC3546__Tcell__.C6E879:RHC6041__Macrophage__.FFFF55))


rownames(nm_cells_ds2) <- nm_cells_ds2$X
nm_cells_ds2 <- gene_name_cleaner(data.to.clean = nm_cells_ds2)
nm_cells_ds2 <- t(nm_cells_ds2)
nm_cells_ds2 <- subset(nm_cells_ds2,
                            select=c(RHC3934__Bcell__.7DEA7B:RHC6187__Macrophage__.FFFF55))
...
keep_rows <- rowSums(tumor_cells_ds2 > 0) > 10
tumor_cells_ds2 <- tumor_cells_ds2[keep_rows,]



keep_rows <- rowSums(nm_cells_ds2 > 0) > 10
nm_cells_ds2 <- nm_cells_ds2[keep_rows,]

tumor_cells_ds2 <- apply(tumor_cells_ds2, c(1,2), as.numeric)


#Looking at the distribution of library sizes
library_size_plot_1 <- ggplot() +
  geom_histogram(aes(x=colSums(tumor_cells_ds2)), bins=100) +
  geom_vline(xintercept = 1000, color='red') +
  theme(panel.background = element_blank(),
        plot.title = element_text(face = "bold", hjust = 0.5))+
  xlab("Library Size Distribution")+
  ylab("Count") + ggtitle("Single-cell Library Size for DESeq2 Tumor")+
  scale_y_continuous(limits=c(0, 60), expand = c(0, 0))
...
#Normalizing the library size
tumor_cells_ds2 <- library.size.normalize(t(tumor_cells_ds2), verbose = TRUE)
tumor_cells_ds2 <- t(tumor_cells_ds2)
tumor_col_names <- colnames(tumor_cells_ds2)
tumor_row_names <- rownames(tumor_cells_ds2)
head(as.data.frame(tumor_cells_ds2))
tumor_cells_ds2 <- sqrt(tumor_cells_ds2)
head(as.data.frame(tumor_cells_ds2))
...
#Denoising the single-cell data and saving the output
tumor_cells_ds2 <- magic_denoiser(sc.data = tumor_cells_ds2,
                                    magic.seed = 123,
                                    magic.solver = 'approximate')

saveRDS(tumor_cells_ds2$denoised_sc_dataframe,
        file = "Outputs/denoised-colon-and-rectal-single-cell-data_deseq2_tumor.rds")



nm_cells_ds2 <- apply(nm_cells_ds2, c(1,2), as.numeric)


#Looking at the distribution of library sizes
library_size_plot_1 <- ggplot() +
  geom_histogram(aes(x=colSums(nm_cells_ds2)), bins=100) +
  geom_vline(xintercept = 1000, color='red') +
  theme(panel.background = element_blank(),
        plot.title = element_text(face = "bold", hjust = 0.5))+
  xlab("Library Size Distribution")+
...
ggsave(filename = "Outputs/single_cell_library_histogram_pre_process_step_1_deseq2_normal.svg",
       device = "svg", units = "cm", dpi = 300, width = 24, height = 24,
       plot = library_size_plot_1)
...
#Normalizing the library size
nm_cells_ds2 <- library.size.normalize(t(nm_cells_ds2), verbose = TRUE)
nm_cells_ds2 <- t(nm_cells_ds2)
tumor_col_names <- colnames(nm_cells_ds2)
tumor_row_names <- rownames(nm_cells_ds2)
head(as.data.frame(nm_cells_ds2))
nm_cells_ds2 <- sqrt(nm_cells_ds2)
head(as.data.frame(nm_cells_ds2))
...
library_size_plot_2 <- ggplot() +
  ...
  theme(panel.background = element_blank(),
        plot.title = element_text(face = "bold", hjust = 0.5))+
  xlab("Library Size Distribution")+
  ylab("Count") + ggtitle("Single-cell Library Size Normalized for DESeq2 Normal")+
  scale_y_continuous(limits=c(0, 35), expand = c(0, 0))

library_size_plot_2

ggsave(filename = "Outputs/single_cell_library_histogram_pre_process_step_2_deseq2_normal.svg",
       device = "svg", units = "cm", dpi = 300, width = 20, height = 20,
       plot = library_size_plot_2)
...
#Denoising the single-cell data and saving the output
nm_cells_ds2 <- magic_denoiser(sc.data = nm_cells_ds2,
                                    magic.seed = 123,
                                    magic.solver = 'approximate')

saveRDS(nm_cells_ds2$denoised_sc_dataframe,
        file = "Outputs/denoised-colon-and-rectal-single-cell-data_deseq2_normal.rds")
...
tumor_num <- rep("tumor", 375)
all_nums <- c(tumor_num, normal_num)
nm_names <- rep(paste("NM",1:ncol(nm_cells_ds2$denoised_sc_dataframe)))
tumor_names <- rep(paste("TM",1:ncol(tumor_cells_ds2$denoised_sc_dataframe)))
all_names <- c(tumor_names, nm_names)

#Merging them together into one large data frame
tumor_cells_ds2_df <- as.data.frame(tumor_cells_ds2$denoised_sc_dataframe)
tumor_cells_ds2_df <- apply(tumor_cells_ds2_df, 2, as.integer)
rownames(tumor_cells_ds2_df) <- rownames(tumor_cells_ds2$denoised_sc_dataframe)
nm_cells_ds2_df <- as.data.frame(nm_cells_ds2$denoised_sc_dataframe)
nm_cells_ds2_df <- apply(nm_cells_ds2_df, 2, as.integer)
rownames(nm_cells_ds2_df) <- rownames(nm_cells_ds2$denoised_sc_dataframe)



#deseq2_df <- merge(tumor_cells_ds2_df, nm_cells_ds2_df)
deseq2_rows <- rownames(deseq2_df)
deseq2_cols <- colnames(deseq2_df)
deseq2_df <- deseq2_df[complete.cases(deseq2_df), ]
deseq2_df <- apply(deseq2_df, c(1,2), as.integer)
...
# save(resOrdered, file = "Outputs/deseq2_top5000_genes_cc_patients.RData")
# 
