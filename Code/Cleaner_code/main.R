...
high_choices <- read.csv("high-dbDEMC-cancer-types.csv")
low_choices <- read.csv("low-dbDEMC-cancer-types.csv")
...
#Now getting pseudotime info from Moncocle3----
cds_output <- cell_dataset_builder(vim.genes = c("VIM", "VIMP"), cell.data = hct116_cells$denoised_sc_dataframe, cell.meta = hct116_cells$cds_gene_names)
...
for (x in hr_calcs_hct116) {
  ...
  current_plot <- km_plotter(km.fit = current_calc$KM, data.source = current_calc$DF, p.value = current_pvalue, plot.title = "MiRNA + SDES HCT116 Cell-line")
