...
risk_score_calculator <- function(my.file=active_coefs.csv, 
  ...
  if(tumor.data==FALSE & n.data==FALSE){
    ...
    #Looping through all of the genes and comparing their express
    for(x in my_genes){
    ...
    #Setting the risk score data frame's rownames
    ...
    converted_df <- as.data.frame(converted_df)
    #Binirizing the risk score into high and low groups
    converted_df$risk <- ifelse(converted_df$risk>median_risk, "high", "low")
    #Fitting our inputs with survfit and seeing how well our risk score
    #stratifies the high versus low risk groups
