...
#mirna_calculator----
mirna_calculator <- function(max.miR.targets             =10,
                             cancer.up                   =TRUE,
                             cancer.type1                =TRUE,
                             cancer.type2                =TRUE,
                             cancer.type3                =TRUE,
  ...
  #Reading in the TargetScan database file----
  target_scan_db <- read.csv(file = "Data/miRNA-data/Predicted_Targets_Context_Scores.default_predictions.txt", sep = '\t')
  ...
  # target_scan_db_sub <- filter(target_scan_db, miRNA %in% common_mirnas)
  # target_scan_db_sub <- target_scan_db_sub[,c("Gene.Symbol","miRNA", "weighted.context...score")]
  # target_scan_db_sub$weighted.context...score <- abs(target_scan_db_sub$weighted.context...score)
  # #View(target_scan_db_sub)
  # #all_genes <- base::unique(target_scan_db_sub$Gene.Symbol)
  # 
  # my_gene_df <- data.frame(genes=unique(target_scan_db_sub$Gene.Symbol), my_score=rep(0, times = length(unique(target_scan_db_sub$Gene.Symbol))))
  # rownames(my_gene_df) <- my_gene_df$genes
  # 
  # for (x in target_scan_db_sub$Gene.Symbol){
  #   #cat(paste0(x, " is being checked currently\n "))
  #   target_scan_db_ind <- filter(target_scan_db_sub, miRNA %in% common_mirnas)
  #   if(x %in% target_scan_db_ind$Gene.Symbol){
  #     #cat(paste0(x, " is present for ", target_scan_db_ind$miRNA, "\n"))
  #     my_gene_df[x, 2] <- my_gene_df[x, 2] + 1
  #     #View(my_gene_df)
  #   }
  #   
  #   
  # }
  # my_gene_df <- my_gene_df[order(my_gene_df$my_score, decreasing = TRUE),]
  # my_gene_df <- my_gene_df[1:max.miR.targets,]
  # #View(my_gene_df)
  # 
  ...
  # miRNA_score <- matrix(data = 0, nrow = length(base::unique(target_scan_db_sub$Gene.Symbol)),
  #                       ncol = length(base::unique(target_scan_db_sub$miRNA)),
  #                       dimnames = list(base::unique(target_scan_db_sub$Gene.Symbol),
  #                                       base::unique(target_scan_db_sub$miRNA)))
  # 
  ...
  #If it does it gets a plus one to that cell. If it does not it moves to next cell
  ...
  #mirna.ranking <- sort(mirna.ranking$my_score, decreasing = TRUE)

  #Return object----
  my_gene_df_finished <- setNames(my_gene_df$my_score, my_gene_df$genes)
  #my_gene_df_finished <-abs(my_gene_df_finished)/sum(abs(my_gene_df_finished))
  #my_gene_df_finished <- sort(my_gene_df_finished, decreasing = TRUE)
