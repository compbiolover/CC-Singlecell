...
save(integrated_gene_lists, file = "Data/integrated_gene_lists.RData")
...
for(x in integrated_gene_lists){
  ...
  integrated_gene_lists[[df_index]] <- current_list
  df_index <- df_index + 1
}

#Saving the sorted lists
save(integrated_gene_lists, file = "Data/sorted_gene_lists.RData")