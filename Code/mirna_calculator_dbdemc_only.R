#Name:mirna_calculator_dbdemc_only.R
#Purpose: Testing miRNA metric with just dbDEMC version 2.0 miRNAs
#Author: Andrew Willems <awillems@vols.utk.edu>
mirna_calculator_dbdemc_only <- function(ts.org          ="Human", 
                             ts.version                  ="7.2",
                             max.miR.targets             =10,
                             cancer.up                   =TRUE,
  ...
  if(cancer.up==TRUE){
    ...
    #dbDEMC high miRNAs only
    dbdemc_mirnas <- dbDEMC_high_miRNAs$miRBase.Update.ID
    ...
    #dbDEMC low miRNAs only
    dbdemc_mirnas <- dbDEMC_low_miRNAs$miRBase.Update.ID
  }
  ...
  dbdemc_mirnas <- dbdemc_mirnas[!dbdemc_mirnas %in% mirna.remove]
  if(length(dbdemc_mirnas)>= length(dbdemc_mirnas[1:max(max.mirnas)])){
    dbdemc_mirnas <- dbdemc_mirnas[1:max.mirnas]
  }else{
    print("There are fewer target miRNAs available than your input.\n 
          Using the largest number of dbDEMC miRNAs for this submission
          to TargetScan")
    dbdemc_mirnas <- dbdemc_mirnas[1:length(dbdemc_mirnas)]
    print(paste0("The number of common mirnas is:", print(length(dbdemc_mirnas))))
  }
  
  #Using the targetScan() function of the hoardeR package to get our gene
  #gene targets for our specific miRNAs
  miRNA_targets <- targetScan(mirna=dbdemc_mirnas,
                              species=ts.org,
                              release=ts.version,
  ...
  miRNA_score$score <- rep(1,times = nrow(miRNA_score))
