...
## Step 9: We are now making the ranked list of genes from the single-cell data for our switchde (`SDE`) metric. 
```{r sde metric}
...
## Step 14b: Getting ideal c-index of random genes on COAD. We do 10 reps of random genes and take the average. Its  c-index performance is 0.63321
...
## Step 14e: Getting ideal gene size for CC Singlecell MM on COAD. It occurs at the 52nd gene size value (2,650 genes). At this value with the 11st combo used (800_1010 with an alpha value of 1 and the 10th value of the weighted linear model [mirna_sde_optimized]) we get a concordance index of 0.7334. 
...
## Step 14e2: Now getting the ideal combo at the optimal gene size (2,650 genes) for CC Singlecell MM on COAD.It is the 19th combo ("700_710") with a concordance index peformance of 0.7351
...
coad_methods_comp_df <- data.frame(Method=c("CC Singlecell MM",
                                            ...
                                            "miRNA", "MAD + SDE" ,
                                            "Random Genes"),
                                   c_index=c(0.7351, 0.7445, 0.7514, 
                                             0.6197,
                                             0.6495,
                                             0.7325,
                                             0.6607, 0.63321))
...
write.csv(coad_methods_comp_df,
          "Data/Reproducible-results/Rmarkdown/Outputs/fig2a_coad_data.csv")
```
...
```{r saving fig2a plot}
#Saving the finished graph in .svg format
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/methods_comp_coad.svg",
...
## Step 14g: Getting Kaplan Meier (KM) curve for top performing CC Singlecell MMS gene set to see if it separates risk groups
```{r fig2c (cc singlecell mms coad km curve)}
cox_df <- readRDS("Data/Reproducible-results/Rmarkdown/Data/coad_df_finished.rds")
fig2c_plot <- risk_score_calculator(my.file = "~/Desktop/All-files-from-paper-draft-update/cc_singlecell_mms_ideal_mirna_fig_s3_coefs.csv",
...
#Saving the KM plot
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/cc_singlecell_mms_coad_km_plot.svg",
...
## Step 14h: Coefficient plot for to CC Singlecell MMS model on COAD data
```{r fig4a data (cc singlecell mms COAD coef plot)}
coad_coef_df <- read.csv("~/Desktop/All-files-from-paper-draft-update/cc_singlecell_mms_ideal_mirna_fig_s3_coefs.csv")
...
coad_coef_df_sub$log_hazard_ratio <- log10(coad_coef_df_sub$hazard_ratio)
#coad_coef_df_sub <- filter(coad_coef_df_sub, Gene != "ZNF705D")
#coad_coef_df_sub <- filter(coad_coef_df_sub, Gene != "UNC5D")
```
...
```{r saving fig 4d (cc singlecell mms COAD coef plot)}
#Saving the coefficients plot
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/cc_singlecell_mms_coad_coef_plot.svg",
...
## Step 14i: Number of active genes for CC Singlecell MMS on COAD
```{r active gene size COAD}
coad_coef_df <- read.csv("~/Desktop/cc_singlecell_mms_ideal_mirna_combo_coefs_of_top_performer_ms.csv")
coad_coef_df <- coad_coef_df[,2:3]
colnames(coad_coef_df) <- c("Gene", "coefs")
coad_coef_df$hazard_ratio <- exp(coad_coef_df$coefs)

num_of_ag_coad <- nrow(coad_coef_df)
...
## Step 16: Ideal MAD gene size on READ. The c-index is 0.7543 at 200 genes
...
## Step 17: Ideal gene size for SDE on READ. Its concordance index is 0.7716 at 1,050 genes 
...
## Step 18: Ideal gene number for miRNA-miRNA target number on READ. C-index is 0.7512 at 2,750 genes when using (1000_1010), 0.7898 at 350 genes when using (1000_110), and 0.7234 at 2,400 genes when using (800_1010)
...
## Step 19: Random gene performance on READ. Its c-index is 0.64411
...
## Step 20: Ideal MAD + SDE performance on READ. It's c-index is 0.8107 at 228 
...
## Step 21a: Ideal CC Singlecell MM gene size on READ. Its ideal weighted index is 11 (0.7898) and occurs at a gene size of 350 genes
...
## Step 21b: Finding the best miRNA-miRNA target combo at the ideal gene size determined in 21a for CC Singlecell MM on READ. Its ideal c-index is 0.802 and occurs at the "1000_610" miRNA-miRNA target combo
...
## Step 21c: Ideal CC Singlecell MS gene size on READ. Its ideal weighted index is 11 (0.7898) and occurs at a gene size of 350 genes 
```{r ideal cc singlecell mm gene size READ}
combo_used <- c("1000_10", "1000_110", "1000_210", "1000_310", "1000_410", "1000_510", "1000_610", "1000_710", "1000_810", "1000_910", "1000_1010",
                 "900_10", "900_110", "900_210", "900_310", "900_410", "900_510", "900_610", "900_710", "900_810", "900_910", "900_1010")
...
##Step 21d: Finding the best miRNA-miRNA target combo at the ideal gene size determined in 21c for CC Singlecell MS on READ. Its ideal c-index is 0.802 and occurs at the "1000_610" miRNA-miRNA target combo. A higher ideal (0.8283) is found at 600 genes and an miRNA-miRNA target value of "400_410". Need to investigate this more 
...
## Step 21e: CC Singlecell MMS on READ: Its best performance is 0.8283. It occurs at gene size of 600. 
...
```{r saving fig2b}
#Saving the finished graph in .svg format
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/methods_comp_read.svg",
...
## Step 21f: Taking top coefficients of CC Singlecell MMS on READ to see if they separate high and low risk groups with KM curves
...
#Saving the KM plot
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/cc_singlecell_mms_read_km_plot.svg",
...
## Step 21g: Taking top READ coefficients 
```{r fig4d}
...
read_coef_df_sub$log_hazard_ratio <- log10(read_coef_df_sub$hazard_ratio)
...
#Saving the coefficients plot
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/cc_singlecell_mms_read_coef_plot.svg",
...
## Step 22a: Now using an unrelated cancer data set (glioblastoma) from the TCGA-GBM project. Because there isn't as many miRNAs for this we don't see big changes in performance between any of our methods.
```{r gbm dataframe}
load("~/Desktop/gbm_df.RData", verbose = TRUE)
```

## Step 22b: GBM single-cell data. We want to subset to just the primary glioblastoma tumors of the 5 patients. It appears that the samples are log2 transformed based on [this]https://www.biostars.org/p/403934/. They have already undergone extensive pre-processing as noted by their authors [here]https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE57872
```{r gbm single-cell data}
gbm_fpkm <- read.csv("Data/Reproducible-results/Rmarkdown/Single-cell-data/Other-cancers/GSE57872_GBM_data_matrix.txt", sep = "\t")
head(gbm_fpkm)

#Subsetting to just the 5 patients
gbm_fpkm <- gbm_fpkm %>% select(X:MGH31_H12)
gbm_fpkm_gene_names <- gbm_fpkm$X
rownames(gbm_fpkm) <- gbm_fpkm$X
head(gbm_fpkm)
gbm_fpkm <- gbm_fpkm %>% select(MGH264_A01:MGH31_H12)
head(gbm_fpkm)
```
```{r magic pre-processing step 1 gbm}
keep_rows <- rowSums(gbm_fpkm > 0) > 10
table(keep_rows)
gbm_fpkm <- gbm_fpkm[keep_rows,]
head(gbm_fpkm)
```

```{r magic denoising of gbm single-cell data}
#Denoising the single-cell data and saving the output
gbm_fpkm <- magic_denoiser(sc.data = gbm_fpkm,
                                magic.seed = 123, magic.solver = 'approximate')
```

```{r saving magic denoising of single-cell gbm data outputs}
saveRDS(gbm_fpkm$denoised_sc_dataframe,
        file = "Data/Reproducible-results/Rmarkdown/Outputs/denoised-gbm-cell-data.rds")

head(as.data.frame(gbm_fpkm$denoised_sc_dataframe))

saveRDS(gbm_fpkm$cds_gene_names,
        file = "Data/Reproducible-results/Rmarkdown/Outputs/denoised-gbm-single-cell-data-gene-names.rds")

head(gbm_fpkm$cds_gene_names)
```


## Step 22c: Doing ZEB1 based pseudotime for GBM. Got EMT markers from this [paper]https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4774466/. We will need to undo the log2 (following [this]https://stackoverflow.com/questions/50787776/how-to-inverse-a-log2-transformation) transformation because `Monocle3` has a log transformation step that I cannot turn off
```{r monocle3 vim pseudotime generation gbm, echo=FALSE}
#Generating the ZEB1 based pseudotime progression with Monocle3
#Based on the expression of the ZEB1 graph we select the bottom-rightmost point
#as the root of our pseudotime as this matches the gradient of ZEB1 expression
#level

gbm_fpkm$denoised_sc_dataframe <- 2^gbm_fpkm$denoised_sc_dataframe

cds_output <- cell_dataset_builder(vim.genes = c("ZEB1"),
                                   cell.data = gbm_fpkm$denoised_sc_dataframe,
                                   norm.flag = "none",
                                   cell.meta = gbm_fpkm$cds_gene_names,
                                   my.root = "Y_92",
                                   my.cds.filename = "Data/Reproducible-results/Rmarkdown/Outputs/cds_ouput_gbm.rds",
                                   my.monocle.graph = "Data/Reproducible-results/Rmarkdown/Outputs/vim_pseudotime_gbm.svg",
                                   my.monocle.graph.genes = "Data/Reproducible-results/Rmarkdown/Outputs/vim_genes_gbm.svg",
                                   my.pt.data.filename = "Data/Reproducible-results/Rmarkdown/Outputs/vim_pseudotime_gbm_data.csv")

cds_output$`PT Graph`
cds_output$`Cell Progression Graph`
```
## Step22d: MAD metric for GBM
```{r mad metric gbm}
#MAD metric for gbm cancer
mad.genes <- mad_calculator(gbm_fpkm$denoised_sc_dataframe)
saveRDS(mad.genes,
        file = "Data/Reproducible-results/Rmarkdown/Outputs/mad_gbm_cancer.rds")

head(mad.genes)
```

## Step 22e: Ideal gene size for MAD metric on GBM. Its top c-index is 0.6489 at a gene size of 200
```{r ideal gene number for mad on GBM}
#Getting ideal gene number for MAD metric on TCGA-COAD
gene_sizes <- seq(100, 3000, 50)
mad_cindices <- rep(0, length(gene_sizes))
...
## Step 22f: SDE metric for GBM
```{r sde metric gbm}
#Switchde metric for gbm cancer
sde.genes <- switchde_calculator(gbm_fpkm$denoised_sc_dataframe,
                                 pseudo.time = cds_output$Pseudotime)

saveRDS(sde.genes,
        file = "Data/Reproducible-results/Rmarkdown/Outputs/sde_gbm_cancer.rds")
...
## Step 22g: Ideal gene size for SDE metric on GBM. It's best c-index is 0.6606 at a gene size of 150. 
```{r ideal gene number for SDE on GBM}
#Getting ideal gene number for SDE metric on TCGA-COAD
gene_sizes <- seq(100, 3000, 50)
sde_cindices <- rep(0, length(gene_sizes))
mad.genes <- readRDS("sde_gbm_cancer.rds")
...
## Step 23: Ideal MAD + SDE c-index on GBM. It's best c-index is 0.7062
```{r getting ideal MAD + SDE gene performance GBM}
gene_sizes <- seq(100, 3000, 50)
...
##Step 24: High miRNA-high miRNA target on GBM. We find that for GBM best performance occurs at low miRNA number 100 and high target number 1010. It has a c-index of 0.6840
```{r ideal gene number for high miRNA-miRNA target size on TCGA-GBM}
...
#Loading the high miRNA-miRNA target file
load("Data/Reproducible-results/Rmarkdown/Data/300_1010_targets.RData",
     verbose = TRUE)
...
for(gs in gene_sizes){
  cox_model <- cox_model_fitter(my.seed = 1,
                                ...
                                tumor.n = FALSE,
                                tumor.m = FALSE,
                                my.filename = paste0("Data/Reproducible-results/Rmarkdown/Outputs/high_mirna_gbm_coefs_",gs,"_genes.csv"))
...
write.csv(mirna_high_cindices_read_df,
          "Data/Reproducible-results/Rmarkdown/Outputs/mirna_high_cindices_gbm_across_gene_size.csv")
```

## Step 24b: The performance is 0.6840
```{r ideal cc singlecell ms gene size GBM}
...
## Step 25: Doing grid search for COAD at different values of alpha to show that LASSO is best regularization
...
## Supplement S2: Heatmaps of COAD and READ miRNA-miRNA metric spaces
```{r fig S2a data}
#COAD Heatmap
combo_used <- c("800_10", "800_110", "800_210", "800_310", "800_410", "800_510", "800_610", "800_710", "800_810", "800_910", "800_1010",
                "700_10", "700_110", "700_210", "700_310", "700_410", "700_510", "700_610", "700_710", "700_810", "700_910", "700_1010",
...
figs3a_df <- read.csv("~/Desktop/All-files-from-paper-draft-update/cc_singlecell_mms_coad_top_cindex.csv")
colnames(figs3a_df) <- c("combo_used", "c_index")
figs3a_df$combo_used <- combo_used
figs3a_df$mirna_num <- rep(seq(800,100, -100), each = 11)
figs3a_df$mirna_target <- rep(seq(10,1010,by=100), times = 8)
```

```{r fig S2a plot}
heatmap_figs3a<- ggplot(data = figs3a_df, aes(x=mirna_num, y=mirna_target, fill=c_index))+
  geom_tile()+
  scale_fill_gradient(low = "white", high = "red")+
...
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/cc_singlecell_mms_grid_search_coad.svg",
       device = "svg", units = "cm", dpi = 300, width = 40, height = 40,
...
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/cc_singlecell_mms_grid_search_read.svg",
       device = "svg", units = "cm", dpi = 300, width = 40, height = 40,
       plot = heatmap_figs3b_finished)
```

## S3: Active gene size graph
...
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/active_gene_set_coad.svg",
...
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/active_gene_set_read.svg",
...
## Step 29: DESeq2 Performance on READ
...
## Step 32: Making barchart of results from all of these different methods compared to our best performing varient of CC Singlecell (Currently CC Singlecell MS) for TCGA-READ (fig3b)
```{r fig3b data}
...
```{r saving fig3b}
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/cc_singlecell_mms_comparison_vs_others_read.svg",
...
## Step 33: scDD Performance on COAD
...
## Step 34: DEsingle on COAD. We use the same DEsingle object that we generated before we just swap out the cox model data frame
```{r DEsingle on COAD}
#For finding the optimal number of genes
...
## Step 35: DESeq2 for COAD. Using previous DESeq2 generated data frame. Just swapping the Cox model data frame
```{r deseq2 on COAD}
...
Step 37: Barchart of all of the methods vs. CC Singlecell for COAD (Fig 3a)
```{r fig3a data}
#For finding the optimal number of genes
gene_sizes <- seq(100, 1930, 50)
...
coad_other_methods_df <- data.frame(method=c("CC Singlecell MMS","DESeq2",
                                             "edgeR", "scDD", "DEsingle"), 
                                    c_index=c(0.7491,max(deseq2_coad_max),
                                              max(edger_coad_max), 
                                              max(scdd_coad_max),
                                              max(deseq2_coad_max)))
```
...
```{r saving fig3a plot}
ggsave(filename = "Data/Reproducible-results/Rmarkdown/Outputs/cc_singlecell_mms_comparison_vs_others_coad.svg",
